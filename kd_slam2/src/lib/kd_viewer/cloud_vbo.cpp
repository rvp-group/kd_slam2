...
namespace kd_slam {
  ...
  PointNormal3fKDCloudVBO::PointNormal3fKDCloudVBO(const std::vector<PointType>& cloud_)
  ...
  void PointNormal3fKDCloudVBO::updateBuffer(const std::vector<PointType>& cloud_) {
    if (cloud_.size()>_capacity) {
      if (_capacity) {
        glDeleteVertexArrays(1, &_gl_vertex_array);
        glDeleteBuffers(1, &_gl_vertex_buffer);
      }
      // copy data on GPU: one PointType (point+normal+rgba) per vertex, interleaved
      ...
      glBufferData(GL_ARRAY_BUFFER,
                   sizeof(PointType) * cloud_.size(),
                   cloud_.empty() ? nullptr : &(cloud_[0]),
                   GL_STATIC_DRAW);
      // specify data layout
      glBindVertexArray(_gl_vertex_array);
      glVertexAttribPointer(0,
                            ...
                            GL_FALSE,
                            sizeof(PointType),
                            (const void*)offsetof(PointType, point));
      ...
      glVertexAttribPointer(1,
                            ...
                            GL_FALSE,
                            sizeof(PointType),
                            (const void*)offsetof(PointType, normal));

      glVertexAttribIPointer(2,
                             1,
                             GL_UNSIGNED_INT,
                             sizeof(PointType),
                             (const void*)offsetof(PointType, rgba));
      _capacity=cloud_.size();
    } else {
      // just copy the data
      glBindBuffer(GL_ARRAY_BUFFER, _gl_vertex_buffer);
      glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(PointType)*cloud_.size(), cloud_.data());
    }
    _size=cloud_.size();
    _normals_vbo.init(cloud_);
  }
  ...
  void PointNormal3fKDCloudVBO::NormalsVBO::init(const std::vector<PointType>& cloud_, float normal_scale) {
    // own, dedicated buffer: (point, point+normal*scale) pairs, drawn as GL_LINES
    std::vector<Eigen::Vector3f> segments;
    segments.reserve(cloud_.size()*2);
    for (const auto& p : cloud_) {
      segments.push_back(p.point);
      segments.push_back(p.point + p.normal*normal_scale);
    }
    if (segments.size()>_capacity) {
      if (_capacity) {
        glDeleteVertexArrays(1, &_gl_vertex_array);
        glDeleteBuffers(1, &_gl_vertex_buffer);
      }
      ...
      glVertexAttribPointer(0,
                            ...
                            (const void*)0);
      _capacity=segments.size();
    } else {
      // just copy the data
      glBindBuffer(GL_ARRAY_BUFFER, _gl_vertex_buffer);
      glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(Eigen::Vector3f)*segments.size(), segments.data());
    }
    _size=segments.size();
  }
