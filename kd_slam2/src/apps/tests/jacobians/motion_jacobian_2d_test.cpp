...
static Scalar evalErrorA(const TransformType& Ta,
                          ...
                          const VelocityType& va, Scalar dt, int i, Scalar h) {
  VelocityType v = va; v(i) += h;
...
static Scalar evalErrorB(const TransformType& Tb,
                          ...
                          const VelocityType& vb, Scalar dt, int i, Scalar h) {
  VelocityType v = vb; v(i) += h;
  Scalar e; JacobianVelocityType J;
...
int main() {
  ...
  for (int t = 0; t < NUM_TESTS; ++t) {
    ...
    const VectorType pa_l  = VectorType(rnd(3), rnd(3));
    const VectorType na_l  = randomUnit();
    const VectorType pb_l  = VectorType(rnd(3), rnd(3));
    ...
    VelocityType va, vb;
    va << rnd(.5), rnd(.5), rnd(.3);
    vb << rnd(.5), rnd(.5), rnd(.3);
    ...
    // A
    {
      ...
      for (int i = 0; i < Traits::VelocityDim; ++i)
